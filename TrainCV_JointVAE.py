...
/home/aleksandr/miniconda3/envs/tf-py38/bin/python TrainCV_JointVAE.py "innerDNNClassifier" "test_folder_VAE_joint" 500 3 3 \
    "sigmoid" "sigmoid" "sigmoid" 0.0001 "ALL" "Betaval" "binary_crossentropy" "binary_crossentropy" 0.0 "BN" "l1_l2" \
        "Not_save" 128 "Optimization" "all_cohorts" 200 "Main_analysis/ML_data/"

conda activate tf-py38
cd /home/aleksandr/Desktop/WORK/Depression_methylation_multicohort_meta
/home/aleksandr/miniconda3/envs/tf-py38/bin/python TrainCV_JointVAE.py "innerDNNClassifier" "test_folder_VAE_joint_RAW" 2000 3 3 \
...
conda activate tf-py38
cd /home/aleksandr/Desktop/WORK/Depression_methylation_multicohort_meta
/home/aleksandr/miniconda3/envs/tf-py38/bin/python TrainCV_JointVAE.py "innerDNNClassifier" "test_folder_VAE_joint_RAW" 600 3 3 \
    "sigmoid" "sigmoid" "sigmoid" 0.0001 "selector_variance_5" "Betaval" "binary_crossentropy" "binary_crossentropy" 0.1 "BN" "l1_l2" \
        "Not_save" 128 "Optimization" "all_cohorts" 200 "Main_analysis/ML_data_raw/"

conda activate tf-py38
cd /home/aleksandr/Desktop/WORK/Depression_methylation_multicohort_meta
/home/aleksandr/miniconda3/envs/tf-py38/bin/python TrainCV_JointVAE.py "innerDNNClassifier" "test_folder_VAE_joint_RAW" 600 3 3 \
    "sigmoid" "sigmoid" "sigmoid" 0.0001 "selector_trees_200" "Betaval" "binary_crossentropy" "binary_crossentropy" 0.1 "BN" "l1_l2" \
        "Not_save" 128 "Optimization" "all_cohorts" 200 "Main_analysis/ML_data/"
...
print('------------------------------------------------------------------------')
print(f'Training Joint VarAE model with inner classifier {MODEL_ARG}')
...
def run_TF_model(X_train,
    ...
    z_log_var = tf.keras.layers.Dense(latent_space, name="z_log_var")(x)
    ...
    class VAEclassifier(tf.keras.Model):
        ...
        def train_step(self, data):
            ...
            with tf.GradientTape() as tape:
                ...
                total_loss = reconstruction_loss + self.alpha*classification_loss + kl_loss
    ...
    # Model compilation
    joint_classif = VAEclassifier(encoder,
                                 decoder, 
                                 dnn_model_classifier, 
    ...
    # train data
    z_mean, z_log_var, encoded_space_train = joint_classif.encoder.predict(X_train, batch_size=BATCH_ARG)
    ...
    # KL-loss train
    kl_loss_train = -0.5 * (1 + z_log_var - tf.square(z_mean) - tf.exp(z_log_var))
    kl_loss_train = tf.reduce_mean(kl_loss_train)
    ...
    # test data
    z_mean, z_log_var, encoded_space_test = joint_classif.encoder.predict(X_test, batch_size=BATCH_ARG)
    ...
    # KL-loss test
    kl_loss_test = -0.5 * (1 + z_log_var - tf.square(z_mean) - tf.exp(z_log_var))
    ...
    final_performance = {
        ...
        "train_kl_loss": float(kl_loss_train),
        "test_kl_loss": float(kl_loss_test),
